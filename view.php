...
$this->menu=array(
    ...
    array(
        ...
        'items'=>array(
            ...
            array(
                'label'=>Yii::t('mc', 'Additional Ports'),
                'url'=>array('ports', 'id'=>$model->id),
                'visible'=>$ports,
                'icon'=>'params'
            ),
        )
...
else
{
    ...
    if (Yii::app()->user->isStaff())
    {
        ...
        $attribs[] = array('label'=>$form->labelEx($model,'suspend_time'), 'type'=>'raw',
            'value'=>$form->textField($model,'suspend_time')
                .' '.$form->error($model,'suspend_time'),
            'hint'=>Yii::t('mc', 'Input suspend time'));
    ...
    $attribs[] = array('label'=>Theme::img('icons/closed.png', '', array('id'=>'advImg')), 'type'=>'raw',
            'value'=>CHtml::link(CHtml::encode(Yii::t('mc', '显示高级设置')), '#', array('id'=>'advTxt', 'onclick'=>'return checkAdv()')));
    if (Yii::app()->user->isStaff())
    {
        ...
        if (@Yii::app()->params['additional_ports'] && isset($settings->user_add_ports))
        {
...
echo CHtml::script('
    advShow = false;
    txtOpen = "'.CJavaScript::quote(Yii::t('mc', '隐藏高级设置')).'";
    txtClosed = "'.CJavaScript::quote(Yii::t('mc', '显示高级设置')).'";
    function checkAdv() {
        advShow = !advShow;
